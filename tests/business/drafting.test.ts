...
describe('AI-C evidence-bound draft creation with simulated HTTP provider', () => {
  let h: Harness;
  let server: Server;
  let mode: 'valid' | 'leak' | 'invented' | 'hold' | 'missing-section' | 'wrong-kind' = 'valid';
  ...
  beforeAll(async () => {
    ...
    server = createServer(async (req, res) => {
      ...
      const response = { statements: [{ id: 's1', text: mode === 'invented' ? '2025年收入100万元' : evidence.text, kind: mode === 'wrong-kind' ? 'source_quote' : 'author_report', evidence_refs: [evidence.id], visibility: 'public', ...(mode === 'missing-section' ? {} : { section: 'later' }) }], unresolved_items: ['未提供具体完成日期'] };
  ...
  it('creates an unconfirmed version, rejects invented facts/private leaks, and discards output after an author edit', async () => {
    ...
    for (const badMode of ['leak', 'invented', 'missing-section', 'wrong-kind'] as const) {
      mode = badMode;
