...
it('maps every imported catalogue slot and only exposes validated current-snapshot captions',async()=>{
 ...
 const insert=async(hash:string,caption:string,year:number|null=null)=>h.ctx.db.insert(aiRuns).values({sourceId:story.source.id,task:'ai_a_extract',status:'succeeded',modelId:'test-model',promptVersion:'test',output:{snapshot_hash:hash,presentation:{category:'职场发展',tags:['职场发展'],year,caption,evidence_refs:[`snapshot:${story.snapshot.id}`]}}});
 await insert('stale','我辞职后开始学习');expect((await read()).json().data.story.cover_caption).toBeNull();
 await insert(story.snapshot.contentHash,'不存在的成功结果');expect((await read()).json().data.story.cover_caption).toBeNull();
 await h.ctx.db.delete(aiRuns).where(eq(aiRuns.sourceId,story.source.id));await insert(story.snapshot.contentHash,'我开始学习软件开发');result=(await read()).json().data.story;expect(result.cover_caption).toBe('我开始学习软件开发');expect(result.cover_year).toBeNull();
