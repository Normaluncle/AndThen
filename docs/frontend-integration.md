...
先读 `docs/PRD.md` 确认产品形态，再按 `docs/goal-audit-v1.2.md` 区分已验收和外部阻塞，接口字段以 `docs/openapi.json` 及运行服务为准。功能前端在 `demo/`，使用 React 19 + Vite 7，可直接在现有页面上调整视觉、布局、组件和动效，无需重新设计后台流程。
...
本地启动和 Docker 部署见 `docs/docker-acceptance.md`。前端不得接触 `.env.local` 的提供方密钥；只调用同源 `/api`。GitHub 同步应包含代码、迁移、测试、PRD 与交接文档，不包含本地凭证、数据库卷或真实参与者材料。
...
运行时模型使用独立 `LLM_BASE_URL`、`LLM_MODEL`、`LLM_API_KEY` 配置；客户端不接触模型密钥。WorkBuddy 是开发代理，与运行时模型分离。真实 Qwen、向量与 memU 项目联调已通过，证据见 `goal-audit-v1.2.md`；真实作者试点及采访质量的系统评估尚未完成。
...
固定身份仅限 local_demo_fixture；本地 Compose 覆盖文件显式开启，公网不得启用。初始化脚本幂等且不覆盖试玩进度。完整 PRD 保留原版章节，`git diff 4f6183d -- docs/PRD.md` 展示 v1.2 增量；PRD-v1.2.md 仅为历史实施摘要。
