...
账号页使用当前标签直接跳转知乎授权，浏览器回调自动签发 HttpOnly Cookie 并返回账号页。前端同源请求带 X-AndThen-Web: 1，启动时通过 /api/me 恢复身份，刷新不需重新登录；原 Bearer 接口保持兼容。不得把凭证写入 localStorage/sessionStorage 或 URL。资料处理同意、断开授权仍为独立操作。
...
访问 http://127.0.0.1:5174，顶部“切换为演示读者 / 切换为模拟作者”使用固定账号，切换后保留历史数据；刷新后自动恢复未过期的会话。发现页已有官方搜索取得的真实摘要；另有三个“【演示】”故事可模拟作者写后续。真实作者没有被测试账号冒认。
