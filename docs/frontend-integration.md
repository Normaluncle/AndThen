...
先读 `docs/PRD-v1.2.md` 确认产品形态，再按 `docs/goal-audit-v1.2.md` 区分已验收和外部阻塞，接口字段以 `docs/openapi.json` 及运行服务为准。功能前端在 `demo/`，使用 React 19 + Vite 7，可直接在现有页面上调整视觉、布局、组件和动效，无需重新设计后台流程。
