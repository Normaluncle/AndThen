...
| T10 skip / pause / resume | `interviews.test.ts` saved answer, paused write rejection, resume; AI history passed to provider | Explicit five-question/skip non-repetition and process restart during interview generation still need evidence |
...
| T20 mixed windows / cohorts | `research.test.ts` scoped aggregate cohorts, null denominator and exclusions | Date filtering, fixed-window response timestamps and deidentified event export not implemented; not passed in full |
...
| GET admin/research-export | Missing. Existing GET research/export is source-scoped aggregate-only; a path alias alone would not satisfy the required date/cohort/event contract |
