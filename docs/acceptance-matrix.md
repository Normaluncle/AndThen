...
| OAuth | 底层 URL/换令牌/用户读取/state 校验/加密六项模拟测试通过；App ID/App Key 和回调安全契约未确认 | HTTP 登录仍未启用；请求持久化、绑定、令牌生命周期与真实授权未完成 |
...
2026-09-13 最新来源衔接检查点：pnpm typecheck 通过；pnpm test 为 201 通过、1 项真实模型 opt-in 跳过；最近候选页面的 pnpm --dir demo build 通过。页面逻辑测试及真实模型 opt-in 的独立通过记录见 implementation-v1.2.md。
