...
| 主动资料提交 | 原文片段/回忆表单、版本阅读、用途同意/撤销；新增版本只允许负责账号，核验后旧导入者不能覆盖 | 数据库写权限回归与前端构建通过；新页面浏览器验收待完成 |
...
| 管理功能页面 | 来源列表、材料版本、核验/同意阅读、核验/审核/建回访表单、测试账户、失败任务列表 | 页面和接口已实现；读取与空态浏览器通过，写操作页面验收及任务重试仍需补齐 |
