...
| 官方能力 | tests/business/zhihu*.test.ts、oauth*.test.ts | 官方搜索及受限数据路径已接入；真实OAuth仍待外部凭证和回调验证 |
...
- 赛事App ID / App Key以及真实OAuth回调与账号授权验收。
- 真实作者参与、内容授权及采访质量评估。
- 共享服务器、域名、HTTPS和正式运行配置；公开代码仓库不是线上产品部署。
