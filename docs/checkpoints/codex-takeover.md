...
## Explicit interview deletion continuation — 2026-09-12

- Added DELETE /api/interviews/:id with verified-owner authorization and explicit confirmation covering derivative publication withdrawal. Migration 0005 adds the interview receipt scope.
- Source-locked transaction cancels and scrubs affected jobs, removes interview messages and AI artifacts, clears derived version text/confirmations while retaining version receipts, withdraws dependent publication and removes outbox/notifications. Source snapshots remain. Repeated/concurrent deletes reuse one receipt; unrelated authors are denied. Remaining active interviews preserve case workflow state.
- Typecheck and affected PostgreSQL suites passed: 4 files / 20 tests. Explicit in-flight HTTP model test confirms late output cannot recreate deleted content. Public withdrawal, source retention, cache cleanup and three concurrent deletion requests are asserted independently.
- Refreshed deletion/frontend/acceptance documentation and source-generated OpenAPI (48 paths, added DELETE method). Running Docker has not yet been rebuilt to include these later migrations. Next major implementation gap: whole-account closure, followed by remaining acceptance matrix checks and final Docker delivery.

## End-user account closure continuation — 2026-09-12

- Added POST /api/me/account-deletion for readers/authors, requiring explicit full-content confirmation and a client-prepared random receipt ID/secret. Transaction removes identity/credentials, own activity, author/imported sources and derivatives, cancels/scrubs jobs, and retains only a content-free receipt with hashed secret. Staff accounts require controlled handover rather than self-service deletion.
- GET /api/deletion-receipts/:id accepts only that dedicated bearer secret and remains usable after session/identity deletion. Client saves credentials before requesting, allowing outcome lookup after response loss; wrong credentials reveal nothing.
- Local-Demo erasure uses short exclusive business-table locks, bounded lock/statement waits, and at most two deadlock retries. No external request occurs under locks. Busy rollback returns 503, not a false success. This intentionally serializes rare deletion and is documented as unsuitable for large-scale erasure workloads.
- Inspection found setInterest previously performed the interest upsert and research write without a shared transaction/source lock. It now does both atomically and rechecks/locks active actor identity inside the transaction. Research observation writes also recheck the actor, preventing stale pre-authenticated text-key activity writes after erasure.
- Typecheck and full suite passed: **31 files / 162 tests**. New tests cover credential/data removal, preserved unrelated sources, independent receipt authorization, ID collision rejection, a previously resolved deleted identity, and deletion racing five authenticated follow writes.
- Updated API/frontend/acceptance docs and generated static OpenAPI: 50 paths. Docker is still on older runtime; final image build/migrations/live fixture/persistence/performance remain required. Remaining matrix gaps include specific classification/qualitative AI evidence and active-model process-crash acceptance; real provider credentials are still absent.

## Current Docker delivery continuation — 2026-09-12

- Built/deployed 12f6b88, applied all six migrations, ran complete HTTP manual fixture and 10-session performance sample (200 writes p95 183.97 ms; 10 async admissions p95 70.71 ms). Nonempty backup restored in isolated database. Fixture survived DB/API/worker restart.
- Found bootstrap file output still echoed a token; fixed in 5f43604 and added a real subprocess/PostgreSQL regression. Built/deployed that revision, verified file-only stdout in-container, consumed the test credential and logged out. Fixture survived forced API/worker recreation and then completed withdrawal/deletion cleanup.
