...
[项目企划 · 中文](docs/然后呢_项目企划书.md) · [English Proposal](docs/AndThen_Project_Proposal_EN.md) · [在线演示](#在线演示) · [运行 Demo](#运行-demo) · [文档导航](docs/README.md)

## 在线演示

| 入口 | 地址 |
| --- | --- |
| 官网 | <https://39.105.229.217/> |
| 演示视频（45 秒 · 1080p · 无需登录，可直接播放或下载） | <https://39.105.229.217/media/andthen-demo.mp4> |
| 评审入口（登录面板会多出「体验演示」：演示读者 / 模拟作者） | <https://39.105.229.217/?judge=1> |

线上环境与 `main` 分支同步部署；演示样本全部带【演示】标记，是虚构材料，服务端可能随时重置演示进度。
...
以下三张为本项目的设计稿；线上 Demo 按同一套设计实现，图中人物、故事、日期与社区数字均为示例。
...
- **一条完整的闭环**：读者对旧回答表达关注 → AI 基于获准使用的材料提出最多五问 → 作者逐条作答、编辑并确认 → 在本站发布「后来」 → 关注者收到站内通知并读到更新。
- **真实的官方数据**：候选来自知乎官方搜索接口，点赞 / 评论 / 热度与发布时间都是接口的真实返回值，不编造数字，也不拿更新时间冒充发布时间。
- **划得清的边界**：未绑定授权的内容不对外公开；AI 只辅助提问与整理，不代写、不编造经历；示例材料一律标注【演示】，不冒充真实作者经历。
- **可核验的工程**：类型检查、单元与集成测试（210 项通过）、前端构建都在提交前跑；线上部署带数据库备份、迁移与健康检查。

当前仍为功能 Demo：已验证范围与未完成事项见[验收矩阵](docs/acceptance-matrix.md)与[本地验收记录](docs/release-acceptance-20260914.md)。

本项目独立开发，不是知乎官方产品；不自动向知乎发文或私信，发布发生在本站。真实作者参与和内容使用需要各自授权，示例材料须标记为虚构。

## 从 Demo 到愿景

**现在**：一个跑在真实官方数据与作者确认闭环之上的可运行 Demo —— 关注、回访、确认、发布、通知这条链路在线上可以完整走通。

**愿景**：不做与知乎并列的新内容平台，而是**一套运行在知乎存量问答资产之上的回访机制** —— 让"半截故事"在原作者自愿的前提下，长出经确认的「后来」。长期看，它可以融入知乎生态（主站通知、关联内容、长期随访），使存量回答随时间继续生长：读者问一句「然后呢？」，作者留下自己的「后来」。

## 运行 Demo

线上演示见上方[在线演示](#在线演示)；下面是在自己机器上跑一套完整环境。需要 Git、Docker 与 Docker Compose。首次构建需要能访问镜像和依赖源。
