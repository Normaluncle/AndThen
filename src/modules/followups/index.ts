...
export const followupsModule: ModuleDefinition = {
  name: 'followups',
  registerRoutes: async (app, ctx) => { await registerFollowupRoutes(app, ctx); await registerDeletionRoutes(app, ctx); await registerReaderDeletionRoutes(app, ctx); await registerInterviewDeletionRoutes(app, ctx); await registerValidationRoutes(app, ctx); await registerDraftingRoutes(app, ctx); },
