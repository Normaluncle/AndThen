...
export async function sourcePresentation(db:Executor,sourceId:string,title:string|null,snapshot?:SourceSnapshotRow):Promise<Presentation>{
 ...
 let category=keywordCategory((title||'')+' '+text),tags=[category],caption:string|null=null;
 ...
 if(parsed.success&&text.includes(parsed.data.caption)&&parsed.data.evidence_refs.every(ref=>ref===`snapshot:${snapshot.id}`)){category=parsed.data.category;tags=parsed.data.tags;caption=parsed.data.caption;}}
 const cover=matchCover(await db.select().from(coverCatalog),category,tags,sourceId+':'+(snapshot?.contentHash||''));
 return {category,tags,cover_id:cover?.id??null,cover_url:cover?.imageUrl??null,cover_caption:caption,cover_status:cover?.imageUrl?'ready':'pending_asset'};
}
