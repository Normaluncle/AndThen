...
import { z } from 'zod';
import { authorMemories, authorVerifications, sourceSnapshots, sources, type AuthorMemoryRecord } from '../../db/schema.js';
...
export async function authorizedMaterials(ctx: ModuleContext, userId: string) {
  const rows = await ctx.db.select({ source: sources }).from(sources)
  ...
  for (const { source } of rows) {
    if (!await hasActiveConsent(ctx.db, source.id, 'external_model_processing', userId)
      || !await hasActiveConsent(ctx.db, source.id, 'private_interview', userId)) continue;
...
export async function refreshMemory(ctx: ModuleContext, userId: string, generation: string, signal?: AbortSignal) {
  ...
  const hash = createHash('sha256').update(JSON.stringify(materials.map(x => [x.source.id, x.snapshot.id, x.snapshot.contentHash]))).digest('hex');
  if (profile.inputHash === hash && profile.status === 'ready') return;
  const records: AuthorMemoryRecord[] = [];
  for (const { source, snapshot } of materials) {
    const text = (snapshot.body ?? snapshot.excerpt ?? '').slice(0, 12000);
    ...
    for (const [i, fact] of parsed.facts.entries()) {
      ...
      records.push({ name: `${snapshot.id}:${i}`, description: fact.summary, content: fact.summary,
  ...
  const currentHash = createHash('sha256').update(JSON.stringify(stillAllowed.map(x => [x.source.id, x.snapshot.id, x.snapshot.contentHash]))).digest('hex');
  if (currentHash !== hash) {
...
export async function recallMemory(ctx: ModuleContext, userId: string, query: string, signal?: AbortSignal) {
  const [profile] = await ctx.db.select().from(authorMemories).where(eq(authorMemories.userId, userId));
  if (!profile?.enabled || profile.status !== 'ready') return { status: 'unavailable', records: [] as AuthorMemoryRecord[] };
  const allowed = await authorizedMaterials(ctx, userId);
  const valid = profile.records.filter(r => allowed.some(x => x.snapshot.id === r.snapshotId && x.snapshot.contentHash === r.contentHash));
  try {
    ...
    let length = 0;
    return { status: 'ready', records: selected.filter(x => (length += x.content.length + x.evidenceText.length) <= 2000) };
  } catch {
    return { status: 'unavailable', records: valid.filter(x => x.preference).slice(0, 5) };
  }
}
