...
const envSchema = z.object({
  ...
  DISCOVERY_DAILY_LIMIT: z.coerce.number().int().min(1).max(50).default(50),
  DISCOVERY_DAILY_SEARCH_LIMIT: z.coerce.number().int().min(1).max(50).default(12),
  DISCOVERY_QUERIES: z.string().default('辞职 转行 后来|考研 经历 后来|创业 经历 后来|搬家 城市 生活'),
  ...
  ZHIHU_REDIRECT_URI: z.preprocess(v=>v===''?undefined:v,z.string().url().optional()),
  ZHIHU_OAUTH_STATE_VERIFIED: booleanish.default('false'),
  ZHIHU_TOKEN_ENCRYPTION_KEY: z.preprocess(v=>v===''?undefined:v,z.string().regex(/^[a-fA-F0-9]{64}$/).optional()),
