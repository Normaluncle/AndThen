...
已实现来源导入、关注、人工核验/复核、案例决策、采访、四阶段模型候选流程、草稿确认、Demo 发布、通知、撤回、来源删除、读者活动删除和研究导出。当前仍有验收缺口，见 [验收矩阵](docs/acceptance-matrix.md)，不能将本仓库描述为完整公开试点。
...
读者/作者整账号注销和独立采访删除已提供，具体范围及独立回执查询见删除接口文档。验收矩阵中的剩余检查及最终 Docker 交付尚未完成。没有真实模型配置或授权样本时，不宣称完成真实动态采访评测或真实作者试点。不购买云资源、不发布公网、不自动发送邀请。
