...
test('all literal SVG icons used in restored pages have actual paths',()=>{const dir=new URL('./design/',import.meta.url);const names=readdirSync(dir).filter(x=>x.endsWith('.jsx'));let checked=0;for(const file of names){const text=readFileSync(new URL(file,dir),'utf8');for(const match of text.matchAll(/<Icon name="([^"]+)"/g)){assert.ok(iconPaths[match[1]],`${file}: missing ${match[1]}`);checked++;}}assert.ok(checked>0);});
