...
import {COPY, coverStyle, interviewProgress, isFixture, liveCount, sectionBlocks, storyTitle} from './ui14.js';
...
export function DiscoverPage({items, query, url, busy, emptyKind, onQuery, onUrl, onSearch, onImport, onOpen, onInterest, onFillImport, reasonSource, reasonCandidate}) {
    ...
    <div className="page page-2" data-screen="01">
      <div className="page-stack">
        <section className="hero">
          ...
          <form className="search-row" onSubmit={(event) => { event.preventDefault(); onSearch(); }}>
        ...
        {emptyKind && !busy && <EmptyState kind={emptyKind} onAction={() => onFillImport()} />}
        {visible.map((item, index) => (
          <article className="card story-card" key={item.source_id || item.url || index}>
              ...
              {item.source_id
                ...
                : (
                  <>
                    ...
                    {item.candidate_id && (
                      <button className="btn-primary" disabled={busy} type="button" onClick={() => onInterest(item)}>
            ...
            {item.source_id && <button className="btn-primary" type="button" onClick={() => onOpen(item.source_id)}>{COPY.then}</button>}
          </article>
        ))}
        <form className="card" onSubmit={(event) => { event.preventDefault(); onImport(); }}>
          ...
          <button className="btn-primary" disabled={busy}>导入并核验</button>
        </form>
      </div>
      <AboutAside />
    </div>
...
export function StoryPage({story, sourceId, storyComments, busy, reasonSource, onInterest, onUnfollow}) {
  if (!story) return <EmptyState kind="no_stories" />;
  ...
  return (
    <div className="page page-2" data-screen="02">
      <article className="card">
        <p className="meta">{story.material_level === 'exact_excerpt' ? '原文片段' : '材料摘要'} · {isFixture(story) ? '测试材料' : '来源材料'}</p>
        <h1>{story.title}</h1>
        <p className="body">{story.text}</p>
        <div className="interest-panel card">
          <div className="brand-mark">?</div>
          <h2>{COPY.then}</h2>
          <p>想知道这段经历后来发生了什么吗？</p>
          <button className="btn-primary" disabled={busy} type="button" onClick={onInterest}>{COPY.followLater}</button>
...
export function FollowingPage({items, busy, emptyKind, onOpen, onDiscover}) {
  ...
  return (
    <div className="page page-2" data-screen="03">
      <div className="page-stack">
        ...
        {emptyKind && !busy && <EmptyState kind={emptyKind} onAction={onDiscover} />}
...
export function NotificationsPage({notifications, emptyKind, busy, onRead, onDiscover}) {
...
export function WorkbenchPage({items, busy, emptyKind, caseId, onCaseId, onOpen, onInterview, onDraft, onDiscover}) {
  const writing = items.filter((item) => item.interview_id || item.draft_id);
  const published = items.filter((item) => item.status === 'published' || item.followup_status === 'published');
  return (
    <div className="page page-2" data-screen="05">
        ...
        {items.map((item) => {
          ...
          return (
            <article className="card" key={item.id}>
              ...
              <button className="btn-primary" type="button" onClick={() => onOpen(item)}>{COPY.willing}</button>
      ...
      <aside className="card">
        ...
        <p>进行中 {writing.length} · 已发布 {published.length}</p>
      </aside>
    </div>
  );
}
    ...
    <div className="page page-2" data-screen="11">
      <section className="card">
        ...
        <div className="interview-actions">
          ...
          <button className="btn-primary" type="button" disabled={busy} onClick={onStart}>开始回访</button>
        </div>
...
export function InterviewPage({session, messages, answer, answerVisibility, busy, interviewState, onAnswer, onVisibility, onFormat, onSave, onSkip, onPause, onResume, onFinish, onRetry, onRefresh, onDraft}) {
  ...
  return (
    <div className="page page-2" data-screen="06">
      <section className="card">
        <p className="progress-label">{progress.label} 问</p>
        ...
        <div className="interview-actions">
          <button className="btn-primary" disabled={busy || !answer || !interviewState.answer} type="button" onClick={onSave}>{COPY.saveContinue} →</button>
    ...
    <div className="page page-2" data-screen="07">
      <section className="card">
        ...
        <div className="interview-actions">
          ...
          <button className="btn-primary" disabled={busy || draftJobPending || !draftState.publish || tooShort} type="button" onClick={onPublish}>{COPY.confirmPublish}</button>
...
export function AdminPage({role, pane = 'manage'}) {
  return (
    <div className="page" data-screen="10">
      <h1>管理后台</h1>
  ...
  return (
    <div className="page page-2" data-screen="12">
      <section className="card">
        ...
        <form className="search-row" onSubmit={(event) => { event.preventDefault(); onImport(); }}>
