...
export function Account({navigate,tab='settings',children,user}){
 ...
 <nav className="d-account-nav"><div className="d-account-user">{user?<span className="d-profile-initial">{user.display_name?.slice(0,1)||<Icon name="me"/>}</span>:<Avatar/>}<span><b>{user?.display_name||'体验访客'}</b><small>把经历写成礼物</small></span></div>{links(false)}<p>有些回答，<br/>不该只停留在过去。<br/>—— 然后呢？</p></nav>
