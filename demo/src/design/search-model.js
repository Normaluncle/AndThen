...
export function coverCaption(item){const raw=item.date||item.published_at;const match=/^(\d{4})[-.年](\d{2})/.exec(raw||item.cover_caption||'');const date=match?`${match[1]}年${Number(match[2])}月`:'';const text=item.cover_caption?.replace(/^\d{4}[-.]\d{2}\s*/, '')||'原回答';return date?[date,text]:['',item.cover_caption||'原回答'];}
