...
export function ConnectedCandidate({id,navigate,user,onLogin}){
 ...
 return <Story story={{...item,author:item.author_name}} navigate={navigate} followSlot={<div className="d-interest"><h2>然后呢？</h2><p>想知道这段经历后来发生了什么吗？</p><Button disabled={action.busy} onClick={()=>{if(!user){onLogin();return;}action.run(async()=>{await api(`/discovery/candidates/${id}/interest`,'PUT',{active:true});navigate('03');});}}>关注后续</Button>{item.url&&<a className="d-link" href={item.url} target="_blank" rel="noreferrer">查看知乎原帖 ↗</a>}<ActionError action={action}/><p className="d-muted">仅为官方搜索摘要，不是全文，也不是作者发布的后来。</p></div>}/>;
}
