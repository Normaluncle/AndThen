...
import {startOwnInterview,startImportedInterview,publishOwnDraft,notificationTarget} from './connected-actions.js';
...
test('an imported link opens its own case before the interview starts',async()=>{
 const calls=[];const request=async(...args)=>{calls.push(args);return args[0]==='/cases'?{case:{id:'case-imported'}}:{session:{id:'session-imported'}};};
 await assert.rejects(()=>startImportedInterview(request,'source-imported',{ownsContent:true,privateInterview:true,modelProcessing:false}));
 assert.equal(calls.length,0);
 const result=await startImportedInterview(request,'source-imported',{ownsContent:true,privateInterview:true,modelProcessing:true});
 assert.equal(result.session.id,'session-imported');
 assert.deepEqual(calls.map(([path])=>path),['/sources/source-imported/consents','/sources/source-imported/consents','/cases','/cases/case-imported/decision','/cases/case-imported/interviews']);
 assert.deepEqual(calls[2][2],{source_id:'source-imported',launch_type:'author_initiated'});
 assert.equal(calls.some(([, ,body])=>body.purpose==='demo_public_display'),false);
});
