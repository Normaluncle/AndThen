...
import {filterHomeStories, homeAction} from './home-data.js';
import './home.css';
...
export function Home({items, query, onQuery, onSearch, onOpen, onInterest, onFillImport, onUrl, busy, onDesignOpen,searchMode=false,results,asideExtra,filterOptions={},onFilters,interactions={}}) {
  ...
  let shown = filterHomeStories(items||[], category, '').filter(item=>{const date=item.date||item.published_at?.slice(0,10);return (!filters.from||(date&&date>=filters.from))&&(!filters.to||(date&&date<=filters.to));});
  ...
  function act(item) {
    ...
    if (action === 'open') onOpen(item.source_id || item.linked_source_id);
  ...
  return <div className={`home-page ${searchMode?'home-search-page':''}`} data-screen="01">
    ...
    <div className="home-columns"><section className="home-feed">
      ...
      {shown.map((item, index) => <article className="home-story" key={item.id || item.source_id || item.candidate_id || index} data-region="feed" data-provenance={item.provenance}>
        <button className="home-cover-link" aria-label={item.title} onClick={()=>act(item)}><StoryCover item={item}/></button>
        <div className="home-story-copy"><h2><button onClick={() => act(item)}>{item.title}</button></h2><p className="home-meta">{item.category || item.author_name || '故事'} · {item.date || item.published_at?.slice(0, 10) || '官方摘要'}</p><p className="home-excerpt">{item.text}</p><span className="home-ellipsis">...</span><div className="home-counts">{(item.site_counts)?.map((count, i) => <span key={i}><HomeIcon name={['like', 'comment', 'star'][i]} />{count}</span>)}</div></div>
