...
/**
 * The account picture. `user.avatar_url` is written by the server only from the
 * verified Zhihu authorization, so it is shown as-is; without it the original
 * initial placeholder stays, and an unbound reader never sees a broken image.
 * Third-party images are requested without a referrer.
 */
export function ProfileAvatar({user, className, fallback}) {
  if (user?.avatar_url) return <img className={['d-profile-photo', className].filter(Boolean).join(' ')} src={user.avatar_url} alt="" referrerPolicy="no-referrer" />;
  return <span className={className}>{user?.display_name?.slice(0,1)||fallback||<HomeIcon name="me" />}</span>;
}
...
export function HomeHeader({onEnter, query, onQuery, onSearch, page='发现', internal=false, onBack, notificationActive=false, unread=0,showWrite=true,showSearch=true,onLogin,user}) {
  return <header className={`home-header ${showSearch?'has-search':'no-search'}`} data-region="chrome-header">
    ...
    {!user&&onLogin?<button className="d-button secondary home-login" onClick={onLogin}>登录</button>:<button className="home-avatar" aria-label="我的" onClick={() => onEnter('账号')}>{user?<ProfileAvatar user={user} className="d-profile-initial" />:<HomeImage crop={[1001, 113, 31, 31]} />}</button>}
  </header>;
}
...
export function Home({items, preview, query, onQuery, onSearch, onOpen, onInterest, onFillImport, onUrl, busy, onDesignOpen, onNavigate, searchMode=false,results,asideExtra,filterOptions={},onFilters,interactions={}}) {
  ...
  let shown = filterByDateRange(filterHomeStories(preview ? homeStories : (items||[]), category, preview ? searched : ''), filters);
